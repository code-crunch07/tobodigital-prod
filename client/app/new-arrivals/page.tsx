    ...
    <div className="group relative bg-white rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] h-full flex flex-col border border-gray-100">
      <div className="relative aspect-square overflow-hidden bg-gray-50">
        ...
        <div className="absolute top-3 left-3 z-20 flex flex-col gap-1.5">
          {discount > 0 ? (
            <span className="inline-flex items-center rounded-full border border-blue-200 bg-blue-50 text-blue-600 text-[10px] font-semibold px-2.5 py-1">Up to {discount}% off</span>
          ) : (
            <span className="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 text-emerald-600 text-[10px] font-semibold px-2.5 py-1">New</span>
          )}
        </div>
        <div className="absolute top-3 right-3 z-20 flex items-center gap-1.5">
          <button type="button" onClick={handleQuickView} className="w-8 h-8 rounded-full bg-white/90 shadow-sm flex items-center justify-center text-gray-400 hover:text-gray-700 transition-colors" title="Quick View"><Eye className="h-3.5 w-3.5" /></button>
          <button type="button" onClick={handleWishlist} className={`w-8 h-8 rounded-full bg-white/90 shadow-sm flex items-center justify-center transition-colors ${isWishlisted ? 'text-rose-500' : 'text-gray-400 hover:text-rose-500'}`} title="Wishlist"><Heart className={`h-3.5 w-3.5 ${isWishlisted ? 'fill-rose-500' : ''}`} /></button>
        </div>
      </div>
...
export default function NewArrivalsPage() {
  ...
  useEffect(() => {
    loadProducts();
  }, [page, selectedCategory, selectedBrands, priceRange, sortBy, selectedRating]);
  ...
  return (
    <>
      <div className="min-h-screen bg-white">
        {/* Top banner – same as category: dark with title + breadcrumb */}
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex items-center justify-between gap-4 mb-6">
            {(() => {
              const hasActiveFilters = selectedCategory !== 'all' || selectedBrands.length > 0 || priceRange[0] !== 1000 || priceRange[1] !== 5000 || selectedRating !== null;
              return (
                <button
                  type="button"
          ...
          <div className="w-full min-w-0">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 mb-6">
                ...
                {Array.from({ length: Math.min(totalPages, 11) }, (_, i) => {
                  let pageNum: number;
                  if (totalPages <= 11) pageNum = i + 1;
                  else if (page <= 6) pageNum = i + 1;
                  else if (page >= totalPages - 5) pageNum = totalPages - 10 + i;
                  else pageNum = page - 5 + i;
                  return (
                    <button
                      key={pageNum}
                      onClick={() => setPage(pageNum)}
                      className={`px-3 py-1 rounded text-sm font-medium ${page === pageNum ? 'bg-[#ff006e] text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                    >
                      {pageNum}
                    </button>
                  );
            ...
            {loading && products.length === 0 ? (
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-center gap-2 mt-8">
                    <button
                      onClick={() => setPage((prev) => Math.max(1, prev - 1))}
                      disabled={page === 1}
                      className={`px-4 py-2 rounded font-medium ${page === 1 ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 hover:bg-gray-100'}`}
                    >
                      Previous
                    </button>
                    {Array.from({ length: Math.min(totalPages, 11) }, (_, i) => {
                      ...
                      return (
                        <button
                          key={pageNum}
                          onClick={() => setPage(pageNum)}
                          className={`px-4 py-2 rounded font-medium ${page === pageNum ? 'bg-[#ff006e] text-white' : 'text-gray-700 hover:bg-gray-100'}`}
                        >
                          {pageNum}
                        </button>
                      );
                    ...
                    <button
                      onClick={() => setPage((prev) => Math.min(totalPages, prev + 1))}
                      disabled={page === totalPages}
                      className={`px-4 py-2 rounded font-medium ${page === totalPages ? 'text-gray-400 cursor-not-allowed' : 'text-gray-700 hover:bg-gray-100'}`}
                    >
                      Next
                    </button>
                  </div>
              ...
              <div className="text-center py-12">
                ...
                <Link href="/shop" className="text-[#ff006e] hover:text-[#d4005a] font-semibold">
                  Browse All Products
                </Link>
              </div>
            )}
        ...
        {/* Filter drawer – same as category page */}
        {filterOpen && (
          <>
                    ...
                    <div className="px-4 pb-4 pt-0 max-h-48 overflow-y-auto">
                      <div className="space-y-2">
                        <label className="flex items-center cursor-pointer">
                          <input type="radio" name="newarrivals-cat" checked={selectedCategory === 'all'} onChange={() => handleCategoryChange('all')} className="mr-3 w-4 h-4 text-[#ff006e] border-gray-300 focus:ring-[#ff006e]" />
                          <span className="text-sm text-gray-700">All</span>
                        </label>
                        {categories.map((cat) => (
                          <label key={cat._id} className="flex items-center cursor-pointer">
                            <input type="radio" name="newarrivals-cat" checked={selectedCategory === cat._id} onChange={() => handleCategoryChange(cat._id)} className="mr-3 w-4 h-4 text-[#ff006e] border-gray-300 focus:ring-[#ff006e]" />
