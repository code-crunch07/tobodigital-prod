    ...
    <div className="min-h-screen bg-white text-gray-900">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid lg:grid-cols-[1.2fr_1fr] gap-12 mb-16">
          {/* Image Gallery */}
          <div className="space-y-4">
            <div className="relative bg-gray-50 border border-gray-200 rounded-2xl p-12 aspect-[4/3] flex items-center justify-center overflow-hidden">
              {discount > 0 && (
                <span className="absolute top-6 right-6 bg-gradient-to-r from-[#FF6B35] to-[#FF8A50] px-4 py-2 rounded-full text-sm font-semibold z-10 animate-pulse text-white">
                  -{discount}%
                </span>
              )}
              <img
              ...
              <div className="grid grid-cols-4 gap-4">
                {images.map((img, index) => (
                  <button
                    ...
                    className={`aspect-square bg-gray-50 border-2 rounded-xl p-4 flex items-center justify-center transition-all ${
                      selectedImage === img
                        ? 'border-[#FF6B35] bg-gray-100 -translate-y-1'
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 pb-6 border-b border-gray-200">
              ...
              <span className="text-gray-500">(2,847 verified reviews)</span>
            </div>

            {/* Price */}
            <div className="bg-gradient-to-br from-[rgba(255,107,53,0.1)] to-[rgba(0,180,204,0.1)] border border-gray-200 rounded-xl p-6">
              <div className="flex items-baseline gap-4 mb-2">
                <span className="text-5xl font-bold text-[#FF6B35]">{formatPrice(currentPrice)}</span>
                {mrp > currentPrice && (
                  <>
                    <span className="text-2xl text-gray-500 line-through">{formatPrice(mrp)}</span>
              ...
              <p className="text-gray-600 text-sm">Inclusive of all taxes | Free Shipping on orders above ₹499</p>
            </div>
            ...
            {bulletPointsArray.length > 0 && (
              <div className="grid grid-cols-2 gap-4">
                {bulletPointsArray.slice(0, 4).map((point, index) => (
                  <div
                    key={index}
                    className="bg-gray-50 border border-gray-200 rounded-lg p-4 flex items-start gap-3 hover:border-[#FF6B35] hover:translate-x-1 transition-all"
                  >
                    <span className="text-2xl flex-shrink-0">🎬</span>
                    <div>
                      <h4 className="font-semibold text-sm mb-1 text-gray-900">{point}</h4>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
              <div className="mb-6">
                <label className="block mb-2 font-semibold text-gray-900">Quantity:</label>
                <div className="inline-flex bg-white border border-gray-300 rounded-lg overflow-hidden">
                  <button
                    onClick={() => setQuantity(Math.max(1, quantity - 1))}
                    className="px-5 py-3 text-xl hover:bg-[#FF6B35] hover:text-white transition-colors text-gray-700"
                  >
                    −
                  </button>
                  <span className="px-6 py-3 font-semibold text-lg min-w-[60px] text-center border-x border-gray-300 text-gray-900">
                    {quantity}
                  </span>
                  <button
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 text-center hover:border-[#00B4CC] hover:-translate-y-1 transition-all">
                <Truck className="h-8 w-8 mx-auto mb-2 text-gray-700" />
                <div className="font-semibold text-sm mb-1 text-gray-900">Free Delivery</div>
                <div className="text-xs text-gray-600">On orders above ₹499</div>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 text-center hover:border-[#00B4CC] hover:-translate-y-1 transition-all">
                <RefreshCw className="h-8 w-8 mx-auto mb-2 text-gray-700" />
                <div className="font-semibold text-sm mb-1 text-gray-900">Easy Returns</div>
                <div className="text-xs text-gray-600">30-day return policy</div>
              </div>
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 text-center hover:border-[#00B4CC] hover:-translate-y-1 transition-all">
                <Shield className="h-8 w-8 mx-auto mb-2 text-gray-700" />
                <div className="font-semibold text-sm mb-1 text-gray-900">1 Year Warranty</div>
            ...
            {/* Pincode Checker */}
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6">
              <h3 className="text-lg font-semibold mb-4 text-gray-900">📍 Check Delivery & Serviceability</h3>
              <div className="flex gap-3">
                <input
                  ...
                  maxLength={6}
                  className="flex-1 bg-white border border-gray-300 rounded-lg px-4 py-3 text-gray-900 placeholder-gray-500 focus:outline-none focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FF6B35]/20"
                />
                <button
            ...
            <div className="space-y-4">
              ...
              {product.hsnCode && (
                <div className="grid grid-cols-[1fr_1.5fr] gap-4 p-4 border-b border-gray-200 hover:bg-[rgba(255,107,53,0.05)]">
                  ...
                  <div className="text-gray-900">{product.hsnCode}</div>
                </div>
              )}
              {product.countryOfOrigin && (
                <div className="grid grid-cols-[1fr_1.5fr] gap-4 p-4 hover:bg-[rgba(255,107,53,0.05)]">
                  <div className="font-semibold text-gray-700">Country of Origin</div>
                  <div className="text-gray-900">{product.countryOfOrigin}</div>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-8">
            {activeTab === 0 && (
              <div>
                ...
                {product.productDescription && (
                  <div
                    className="prose max-w-none prose-img:max-w-full prose-img:h-auto mb-4"
                    dangerouslySetInnerHTML={{ __html: product.productDescription }}
                  />
                )}
                {!product.productDescription && (
                  <p className="text-gray-700 leading-relaxed mb-4">No description available.</p>
                )}
                {bulletPointsArray.length > 0 && (
            ...
            {activeTab === 1 && (
              <div>
                ...
                {compatibleDevicesArray.length > 0 ? (
                  ...
                  <p className="text-gray-700">Compatibility information not available.</p>
                )}
              </div>
            )}
                ...
                {includedComponentsArray.length > 0 ? (
                  <ul className="space-y-3">
                    {includedComponentsArray.map((component, i) => (
                      <li key={i} className="flex justify-between items-center p-3 border-b border-gray-200">
                        <span className="text-gray-700">✓ {component}</span>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="text-gray-700">Package contents information not available.</p>
                )}
              </div>
            )}
        ...
        {similarProducts.length > 0 && (
          <div className="mt-16">
            <h2 className="text-3xl font-bold text-center mb-8 relative text-gray-900">
              You May Also Like
              ...
              {similarProducts.map((item) => {
                ...
                return (
                  <Link key={item._id} href={`/product/${item._id}`} className="group">
                    <div className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden hover:border-[#FF6B35] hover:-translate-y-2.5 hover:shadow-[0_15px_40px_rgba(255,107,53,0.2)] transition-all">
                      ...
                      <div className="p-4">
                        ...
                        <h3 className="font-semibold text-sm mb-2 line-clamp-2 text-gray-900 group-hover:text-[#FF6B35] transition-colors">
                          {item.itemName}
                        </h3>
                        ...
                        <div className="flex items-baseline gap-2 mb-3">
                          ...
                          {itemMrp > itemPrice && (
                            <span className="text-sm text-gray-500 line-through">{formatPrice(itemMrp)}</span>
                          )}
                        </div>
                        <button className="w-full bg-transparent border border-gray-300 text-gray-700 py-2 rounded-lg font-semibold hover:bg-[#FF6B35] hover:border-[#FF6B35] hover:text-white transition-all">
                          View Details
                        </button>
                      </div>
                    </div>
